...
- Voice is **per-directory and OFF by default.** A SessionStart hook reports the state: `[tts] Voice output is ON for this directory` or `OFF`. The `/tts` skill toggles the current directory (the choice persists under `~/.grok/.tts-dirs/`).
